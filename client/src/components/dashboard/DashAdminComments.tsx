...
const DashAdminComments = () => {
    ...
    useEffect(() => {
        ...
        getDashboardComment().then((msg) => {
            if (msg.response?.ok) {
                setComments(msg.data.comments);
                setTotal(msg.data.total);
                setLastMonthTotal(msg.data.lastMonthCommentsCount);
    ...
    const handleDeleteComment = async () => {
        setShowModal(false);
        if (!currentUser || !selectedComment || !currentUser.admin) {
            return;
        }
...
export default DashAdminComments;
